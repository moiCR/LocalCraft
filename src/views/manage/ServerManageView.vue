...
import ServerSettingsModal from "../../components/servers/ServerSettingsModal.vue";
...
<template>
    <div class="h-full w-full flex flex-col gap-4">
        <section class="flex flex-row gap-4 items-center">
            ...
            <nav class="flex flex-row gap-2">
                <ServerNavItem
                    ...
                    :title="'Console'"
                    :icon="SquareTerminal"
                    :active="route.path.includes('console')"
                />
                <ServerNavItem
                    :to="{ name: 'server-files' }"
                    :title="'Files'"
                    :icon="File"
                ...
                <ServerNavItem
                    ...
                    :title="'Mods'"
                    :icon="Package"
                    :active="route.path.includes('mods')"
                />
            </nav>
        </section>
        <section class="flex flex-row justify-between items-center">
            <div class="flex flex-col">
                ...
                <p class="text-md text-white/50">
                    {{ server?.ram }} RAM - Java {{ server?.java_version }}
                </p>
            </div>
        ...
        <ServerSettingsModal
            ...
            :server="server"
            :anchor-el="(settingsButtonRef as any)?.$el ?? settingsButtonRef"
            :layout-id="`settings-server-${server?.id}`"
